...
function ProgrammerAttendance() {
    ...
    useEffect(() => {
        ...
        axios.post(api_url+"read_check_approval.php", { date: date, employee: localStorage.getItem('employee_id') })
            .then((res) => {
                ...
                setCheckIn(res.data.data[0].check_in);
            })
          }, 1000);
    ...
    // useEffect(()=>{
    //     axios.post(api_url+"read_check_approval.php",{date:date,check_in:checkIn})
    //     .then(()=>{
    //         localStorage.setItem("check_out","none")
    //         localStorage.setItem("check_in","inline")
    //         if(localStorage.getItem("check_in")==="inline")
...
export default ProgrammerAttendance
